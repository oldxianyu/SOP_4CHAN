...
- **AI 配置**：每个登录账号可维护自己的 AI API Key、Base URL、模型名称和调用协议；未配置时使用 `hydee` 管理员配置兜底。
- **用户管理**：管理员可查看门户用户、角色、启停状态、最近登录和数据使用情况，并可编辑基础账号信息。
...
- 每个登录账号可维护自己的 AI 配置；管理员可查看全部复盘报告、测评数据和用户列表。
- 管理员可在“用户管理”页编辑用户姓名、手机号、邮箱、角色和启停状态；系统会阻止当前管理员把自己停用或降级。
