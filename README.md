...
企微扫码入口提供受控的 token 交接方式：用户先通过企业微信扫码进入新零售管理平台，再把可用于接口请求的 Authorization token 显式提交给服务器。服务器只用该 token 调用四季蝉业务接口导出复盘数据，不抓取第三方 Cookie，不把 token 写入报告产物或 GitHub；若后续海典后台支持回调本站或 token 交换接口，可升级为扫码后自动交接。
