...
## 2026-06-11 更新：动销率真实口径

AI 复盘的“活动执行情况”新增真实基础档案口径，避免只取活动汇总接口里的平均动销率：

- 门店动销率：动销门店数 / 启用且非仓库门店总数。门店总数来自新零售后台 `merchant/institution/list` 对应的基础档案接口。
- 人员动销率：动销员工数 / 符合随心看条件的员工总数。员工总数来自 `merchant/personManager/index` 对应接口，并按员工账号、在职、随心看启用、角色为店员/店长/运营/区域经理筛选。
- 商品动销率：动销商品数 / 四季蝉商品总数。商品总数优先来自四季蝉概览指标，缺失时使用活动/销售商品唯一数兜底。

技术实现上，门户内置四季蝉导出器会生成 `operationBase` 摘要，并把分子、分母、比例和来源写入：

- 历史报告 payload 的 `summary.operationBase`
- AI 输入摘要 `operationInsights.metrics.movingRates`
- Excel 工作簿的“动销率口径”页签
- 接口诊断与数据源状态

`SOP_4CHAN` 与独立仓库 `sijichan-shuju` 使用同一套计算口径，方便线上报告和离线数据包保持一致。
